export const simplebuttonDialogConfig = function (editor) {
  ...
  return {
    ...
    contents: [
      {
        ...
        elements: [
          ...
          },
          {
            ...
            validate: CKEDITOR.dialog.validate.regex(
              /^\+?(0|[1-9]\d*)$/,
              'Button vertical padding is not valid.',
            ),
            setup: function (element, preview) {
              this.preview_button = preview;
          ...
          },
          {
            ...
            setup: function (element) {
              ...
              custom_color_button.setAttribute(
                'onclick',
                'var colors_table = this.parentNode.querySelector(".colors-table"); if(colors_table.style.display == "none") colors_table.style.display = "block"; else colors_table.style.display = "none";',
              );
              var color_text_input = _table.findOne('.color-text-input');
              color_text_input.setAttribute(
                'value',
                element.getAttribute('style').split('background-color:')[1].split(';')[0],
              );
              color_text_input.setAttribute(
                'onchange',
            ...
            commit: function (element) {
              var background = this.getElement().getAscendant('table').findOne('.color-text-input')
                .$.value;
    ...
    onShow: function () {
      ...
      if (!element || !element.hasClass('simple-button-plugin')) {
        ...
        var style_button =
          'display:inline-block;background-color:#27AE61;border:1px solid #27AE61;color:#fff !important;padding:5px 10px;border-radius:5px;font-size:14px;text-decoration: none !important; cursor: pointer;';
    ...
    onOk: function () {
      ...
      if (this.insertMode) editor.insertElement(simple_btn);
    },
  };
};

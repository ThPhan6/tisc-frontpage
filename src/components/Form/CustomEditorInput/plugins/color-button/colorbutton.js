...
export const loadColorButtonPlugin = (CKEDITOR) => {
  /**
  ...
  var ColorBox, ColorHistoryRow, ColorHistory;
    ...
    init: function (editor) {
      ...
      function addButton(options) {
        var name = options.name,
          ...
          colorData = { type: type },
          defaultColorStyle = new CKEDITOR.style(config['colorButton_' + type + 'Style'], {
            color: 'inherit',
          }),
          clickFn = createClickFunction(),
          history = ColorHistory.getRowLimit(editor)
            ? new ColorHistory(editor, type == 'back' ? 'background-color' : 'color', clickFn)
            : undefined,
          panelBlock;
        ...
        editor.ui.add(name, CKEDITOR.UI_PANELBUTTON, {
          ...
          onOpen: function () {
            ...
            if (config.colorButton_enableAutomatic) {
              panelBlock.element
                .findOne('#' + colorBoxId)
                .setStyle('background-color', automaticColor);
            }
        ...
        function createColorStyleDefinition() {
          ...
          colorStyleDefinition.childRule =
            type == 'back'
              ? function (element) {
        ...
        function setColor(color, colorName, colorHistory) {
          ...
          var colorStyle =
            !CKEDITOR.tools.isEmpty(colorStyleVars) &&
            new CKEDITOR.style(createColorStyleDefinition(), colorStyleVars);
      ...
      function renderColors(colorBoxId, clickFn, historyLength) {
        ...
        for (var i = 0; i < colors.length; i++) {
          if (i % editor.config.colorButton_colorsPerRow === 0) output.push('</tr><tr>');
        ...
        function generateAutomaticButtonHtml(output) {
          output.push(
            ...
            '<tr>',
            '<td colspan="',
            editor.config.colorButton_colorsPerRow,
            '" align="center">',
    ...
    statics: {
      ...
      normalizeColor: function (color) {
        ...
        return CKEDITOR.tools
          .normalizeHex('#' + CKEDITOR.tools.convertRgbToHex(color || ''))
          .replace(/#/g, '');
      },
    },
  ...
  ColorHistory = CKEDITOR.tools.createClass({
    ...
    statics: {
      renderContainer: function (output, editor) {
        output.push(
          '</tbody><tbody class="cke_colorhistory" style="display:none;">',
          '<tr>',
          '<td colspan="',
          editor.config.colorButton_colorsPerRow,
          '" align="center">',
    ...
    _: {
      countColors: function () {
        var spans = CKEDITOR.tools.getStyledSpans(this.cssProperty, this.editor.editable()),
          colorOccurrences = CKEDITOR.tools.array.reduce(
            spans,
      ...
      getHexCode: function (span, cssProperty, list) {
        ...
        return color in list
          ? list[color].substr(1)
          : ColorBox.normalizeColor(span.getComputedStyle(cssProperty)).toUpperCase();
      },

...
export const TeamProfilesEntryForm: FC<TeamProfilesEntryFormValue> = ({ value, onChange }) => {
  ...
  const handleChooseRadioContentType = (radioKey: typeRadio, radioValue: RadioValue) => {
    /// overwrite department data only radio list showed otherwise don't need
    if (radioKey === 'department' && visible) {
      onChange({ ...value, department: { value: radioValue.value, label: radioValue.label } });
    } else {
  ...
  return (
    <EntryFormWrapper customClass={styles.entry_form}>
      {/* First Name */}
      <FormGroup label="First Name" required={true} layout="vertical" formClass={styles.form_group}>
      ...
      {/*  Last Name */}
      <FormGroup label="Last Name" required={true} layout="vertical" formClass={styles.form_group}>
        <div className={styles.form_group__content}>
          <CustomInput
            borderBottomColor="mono-medium"
      ...
      >
        <div className={styles.form_group__content}>
          <CustomInput
            ...
            disabled
          />
          <ActionRightIcon className={styles.action_right_icon} />
        </div>
      </FormGroup>

      {/* Department */}
      <FormGroup label="Department" required={true} layout="vertical" formClass={styles.form_group}>
        <div className={styles.form_group__content} onClick={handleShowDepartmentList}>
          <CustomInput
            borderBottomColor="mono-medium"
            placeholder="select from the list"
            disabled
          ...
          />
          {visible ? (
            <DropUpIcon className={styles.drop_icon} />
          ) : (
            <DropDownIcon className={styles.drop_icon} />
          )}
        </div>
      ...
      {/* Work Email */}
      <FormGroup label="Work Email" required={true} layout="vertical" formClass={styles.form_group}>
        <div className={styles.form_group__content}>
          <CustomInput
            borderBottomColor="mono-medium"
